...
# Overview
For the completetion of this assignment, we are going to develop a machine learning model to predict the type of exercise performed based on different types of measurements given. We are given two files: **training** and **testing**. The first file is the training file, used to develop our algorithm, and the second is used to make our final predictions.
...
```{r}
#setwd("D:\\path\\to\\Working\\directory")
...
```{r eval= FALSE}
#Install Packages if not alrady installed
...
we now load the test.R Data file
```{r}
load('test.RData')
z$table
z$overall[1]
